...
def _build_node_system_prompt(node: NodeContract, contract: GameContract) -> str:
    ...
    parts.append(
        ...
        "ALWAYS include the type annotation on @onready unique-name references "
        "(e.g. @onready var score_label: Label = %ScoreLabel). "
        "The type annotation is required — the pipeline uses it to determine "
        "the correct node type in the .tscn file. Without it, the node defaults "
        "to Node2D and any script-specific methods will be missing. "
