...
def _get_template_asset_paths(perspective: str) -> list[str]:
    """Return all res:// paths provided by the base template.

    These files exist on disk after template copy but are NOT generated by the
    LLM.  The verifier needs to know about them to avoid false "missing file"
    reports.
    """
    particles = PARTICLE_PATHS_3D if perspective == "3D" else PARTICLE_PATHS_2D
    paths: list[str] = []
    paths.extend(SHADER_PATHS.values())
    paths.extend(PALETTE_PATHS.values())
    paths.extend(particles.values())
    paths.extend(CONTROL_SNIPPET_PATHS.values())
    return paths
        ...
        *,
        ...
        try:
            ...
            for iteration in range(1, MAX_ITERATIONS + 1):
                ...
                # Collect all known asset paths: generated + template-provided
                ...
                # Include template-provided files so verifier doesn't flag them
                generated_assets.extend(
                    _get_template_asset_paths(spec.perspective)
                )
