...
NOTE: A list of generated binary assets (PNG sprites, GLB models) is provided \
separately. These files exist on disk even though their contents are not shown. \
Do NOT flag load() or preload() calls to these asset paths as missing references.
...
def _build_verifier_prompt(
    ...
    if generated_assets:
        ...
        asset_block = (
            f"\n\nGenerated Assets ({len(generated_assets)} total):\n"
            f"The following asset files were generated by the asset pipeline and "
            f"exist on disk. Do NOT flag these as missing.\n{asset_list}\n"
        )
