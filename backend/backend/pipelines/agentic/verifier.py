...
NOTE: A list of known asset files (generated sprites/models AND template-provided \
files like shaders, palettes, particle scenes, and control snippets) is provided \
separately. These files exist on disk even though their contents are not shown. \
Do NOT flag load() or preload() calls to any of these paths as missing references.
...
def _build_verifier_prompt(
    ...
    Args:
        ...
        generated_assets: List of asset paths (e.g. "res://assets/sprites/knight.png")
            that were successfully generated by the 2D/3D asset pipeline. These
            files exist on disk but are not included in ``files`` (binary data).
    ...
    asset_block = ""
    if generated_assets:
        asset_list = "\n".join(f"  - {a}" for a in generated_assets)
        asset_block = (
            f"\n\nKnown Asset Files ({len(generated_assets)} total):\n"
            f"The following files exist on disk (generated sprites/models AND "
            f"template-provided shaders, palettes, particle scenes, control snippets). "
            f"Do NOT flag load() or preload() calls to any of these as missing.\n"
            f"{asset_list}\n"
        )
